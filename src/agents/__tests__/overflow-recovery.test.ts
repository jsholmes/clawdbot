...
const baseConfig: OverflowRecoveryConfig = {
  ...DEFAULT_OVERFLOW_RECOVERY_CONFIG,
  enabled: true,
  twoPass: false, // unit tests use single-pass; two-pass is exercised in live tests
};
...
describe("preTruncateTranscript", () => {
  ...
  it("truncates middle and inserts marker when over limit", () => {
    const msgs: TranscriptMessage[] = Array.from({ length: 20 }, (_, i) => ({
      role:
        i % 2 === 0
          ? ("user" satisfies TranscriptMessage["role"])
          : ("assistant" satisfies TranscriptMessage["role"]),
  ...
  it("preserves system prompt", () => {
    const tailMessages: TranscriptMessage[] = Array.from({ length: 10 }, (_, i) => ({
...
describe("performOverflowRecovery", () => {
  it("returns success when model call succeeds", async () => {
    ...
    expect(mockModel).toHaveBeenCalled(); // called once (single-pass) or twice (two-pass)
  });
...
describe("loadSessionTranscriptMessages", () => {
  it("includes tool and system history from session JSONL", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "overflow-recovery-"));
    ...
    const messages = await loadSessionTranscriptMessages(file);
...
describe("rewriteSessionWithSummary", () => {
  it("preserves session header and rewrites body to a single summary message", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "overflow-recovery-"));
    const file = path.join(dir, "session.jsonl");
    const header = { type: "session", version: 3, id: "s1", timestamp: "2026-03-08T00:00:00.000Z" };
    const body = {
      type: "message",
      id: "m1",
    ...
    await rewriteSessionWithSummary({
    ...
    expect(JSON.parse(lines[0])).toMatchObject(header);
    const rewritten = JSON.parse(lines[1]);
    expect(rewritten.type).toBe("message");
    expect(rewritten.message.role).toBe("assistant");
    expect(rewritten.message.content[0].text).toContain("summary text");
    expect(rewritten.message.content[0].text).toContain("SESSION CONTEXT");
  });
});
